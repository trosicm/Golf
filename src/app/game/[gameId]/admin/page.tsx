...
function getPlayerDisplayName(player: any, invite?: any) {
  return player?.name ?? player?.display_name ?? player?.full_name ?? player?.nickname ?? invite?.name ?? invite?.email ?? player?.email ?? player?.id ?? "Player";
}

function getWalletBalance(wallet: any) {
  return Number(wallet?.current_balance ?? wallet?.balance ?? wallet?.amount ?? wallet?.total ?? 0);
}
...
function createWalletPatch(wallet: any, amount: number) {
  ...
  if ("amount" in wallet) patch.amount = amount;
  if ("total" in wallet) patch.total = amount;

  if (Object.keys(patch).length === 0) patch.current_balance = amount;
...
function createHolePatch(hole: any, draft: { par: string; strokeIndex: string; value: string }) {
  ...
  if (!("stroke_index" in hole) && !("strokeIndex" in hole) && !("si" in hole) && Number.isFinite(strokeIndex)) {
    patch.stroke_index = strokeIndex;
  }
...
export default function AdminPage() {
  ...
  const loadData = async () => {
    ...
    if ((inviteRow.role || "").toLowerCase() !== "admin") {
      setError("Admin access required for this panel.");
      setLoading(false);
      return;
    }
    ...
    loadedHoles.forEach((hole: any) => {
      ...
      nextHoleDrafts[String(holeNumber)] = {
        par: String(hole.par ?? ""),
        strokeIndex: String(getStrokeIndex(hole) ?? ""),
        value: String(getHoleValue(hole)),
      };
    });
    setHoleDrafts(nextHoleDrafts);

    if (loadedInvites.length <= 1) {
      setWarning("Only your invite is visible. If other players do not appear, Supabase RLS must allow admins to read all invites for this match.");
    }
  ...
  const playerRows = useMemo(() => {
    ...
    return players.map((player, index) => {
      ...
      return {
        ...
        teamName: `Team ${Math.floor(index / 2) + 1}`,
        wallet,
        balance: getWalletBalance(wallet),
      };
    });
  ...
  const saveMatchSettings = async () => {
    ...
    const { error: updateError } = await supabase
      .from("games")
      .update({ data: nextData, status: matchStatus })
      .eq("id", gameId);

    if (updateError) {
      setError(`Could not update match: ${updateError.message}`);
      setSaving(false);
      return;
    }

    setSuccess("Match settings updated.");
    setSaving(false);
    await loadData();
  };
  ...
  const saveWallet = async (playerId: string, amountOverride?: number) => {
    ...
    if (wallet?.id) {
      ...
      if (updateError) {
        setError(`Could not update wallet: ${updateError.message}`);
        setSaving(false);
        return;
      }
    } else {
      const insertAttempts = [
        ...
        { game_id: gameId, player_id: playerId, amount },
      ];
      ...
      if (!inserted) {
        setError(`Could not create wallet: ${insertError?.message || "Unknown error"}`);
        setSaving(false);
        return;
      }
    }
  ...
  const quickAdjustWallet = async (playerId: string, delta: number) => {
    const current = Number(walletDrafts[playerId] ?? 0);
    const next = current + delta;
    setWalletDrafts((drafts) => ({ ...drafts, [playerId]: String(next) }));
    await saveWallet(playerId, next);
  };

  const saveHole = async (hole: any) => {
    const holeNumber = getHoleNumber(hole);
    const draft = holeDrafts[String(holeNumber)];
    if (!draft) return;
    ...
    if (String(hole.id).startsWith("fallback-")) {
      const insertAttempts = [
        {
          ...
          stroke_index: Number(draft.strokeIndex),
          base_value: Number(draft.value),
        },
        {
          game_id: gameId,
          hole_number: holeNumber,
      ...
      let inserted = false;
      for (const payload of insertAttempts) {
        const { error: attemptError } = await supabase.from("holes").insert(payload);
        if (!attemptError) {
          inserted = true;
          break;
        }
      ...
      if (!inserted) {
        setError(`Could not create hole ${holeNumber}: ${insertError?.message || "Unknown error"}`);
        setSaving(false);
        return;
      }
    } else {
      const patch = createHolePatch(hole, draft);
      const { error: updateError } = await supabase.from("holes").update(patch).eq("id", hole.id);

      if (updateError) {
        setError(`Could not update hole ${holeNumber}: ${updateError.message}`);
        setSaving(false);
        return;
      }
    }

    setSuccess(`Hole ${holeNumber} updated.`);
    setSaving(false);
    await loadData();
  };
  ...
  const seedDefaultHoles = async () => {
    ...
    for (const hole of DEFAULT_HOLES) {
      const existing = holes.find((item) => getHoleNumber(item) === hole.hole_number && !String(item.id).startsWith("fallback-"));
      ...
      const { error: insertError } = await supabase.from("holes").insert({
        ...
        base_value: hole.base_value,
      });

      if (insertError) {
        const { error: fallbackError } = await supabase.from("holes").insert({
          game_id: gameId,
          hole_number: hole.hole_number,
          par: hole.par,
          stroke_index: hole.stroke_index,
        ...
        if (fallbackError) {
          setError(`Could not seed hole ${hole.hole_number}: ${fallbackError.message}`);
          setSaving(false);
          return;
        }
      }
    ...
    setSuccess("Default holes created.");
    setSaving(false);
    await loadData();
  };
  ...
  return (
    <div className="p-3 max-w-6xl mx-auto pb-10">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-1 text-xs font-black uppercase tracking-[0.18em] text-[var(--gr-gold)]">Admin Control</div>
          <h1 className="text-3xl font-black">Match Admin Panel</h1>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <section className="card lg:col-span-1">
          <div className="mb-4">
            <div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">Match settings</div>
            <h2 className="text-xl font-black">Vuelta</h2>
          </div>
          <label className="mb-3 block text-sm font-bold">
            Match name
            <input className="mt-2 w-full" value={matchName} onChange={(event) => setMatchName(event.target.value)} />
          </label>
          <label className="mb-4 block text-sm font-bold">
            Status
            <select className="mt-2 w-full" value={matchStatus} onChange={(event) => setMatchStatus(event.target.value)}>
          ...
          <button className="btn btn-gold w-full" disabled={saving} onClick={saveMatchSettings}>Save match</button>
        </section>

        <section className="card lg:col-span-2">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              <div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">Wallet control</div>
              <h2 className="text-xl font-black">Cartera por jugador</h2>
              <p className="mt-1 text-sm text-[var(--gr-text-muted)]">Estos saldos son los que luego deben mostrarse en Live Game y Perfil.</p>
            </div>
            <div className="text-right">
              <div className="text-xs text-[var(--gr-text-muted)]">Total wallets</div>
              <div className={totalWalletBalance < 0 ? "text-2xl font-black text-danger" : "text-2xl font-black text-success"}>{formatMoney(totalWalletBalance)}</div>
            </div>
          </div>
              ...
              <div key={player.rowKey} className="rounded-2xl border border-[var(--gr-border)] bg-black/10 p-4">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-[var(--gr-text-muted)]">{player.email}</div>
                    <div className="mt-1 text-xs text-[var(--gr-gold)]">{player.teamName} · {player.role}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs uppercase text-[var(--gr-text-muted)]">Current</div>
                    <div className={Number(walletDrafts[player.id] ?? 0) < 0 ? "text-xl font-black text-danger" : "text-xl font-black text-success"}>
                      {formatMoney(Number(walletDrafts[player.id] ?? 0))}
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-3 flex gap-2">
                  <input
                    type="number"
                    className="w-full"
                    value={walletDrafts[player.id] ?? "0"}
                    onChange={(event) => setWalletDrafts((drafts) => ({ ...drafts, [player.id]: event.target.value }))}
                  />
                  <button className="btn btn-gold whitespace-nowrap" disabled={saving} onClick={() => saveWallet(player.id)}>Set</button>
                </div>

                <div className="grid grid-cols-4 gap-2">
                  <button className="btn btn-secondary" disabled={saving} onClick={() => quickAdjustWallet(player.id, -50)}>-50</button>
                  <button className="btn btn-secondary" disabled={saving} onClick={() => quickAdjustWallet(player.id, -10)}>-10</button>
                  <button className="btn btn-secondary" disabled={saving} onClick={() => quickAdjustWallet(player.id, 10)}>+10</button>
                  <button className="btn btn-secondary" disabled={saving} onClick={() => quickAdjustWallet(player.id, 50)}>+50</button>
                </div>
              </div>
      ...
      <section className="card mt-4">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">Hole economy</div>
            <h2 className="text-xl font-black">Valores de hoyo</h2>
            <p className="mt-1 text-sm text-[var(--gr-text-muted)]">Total base actual: <span className="font-black text-[var(--gr-gold)]">€{totalHoleValue.toFixed(0)}</span></p>
          </div>
          <button className="btn btn-gold" disabled={saving} onClick={seedDefaultHoles}>Create default holes</button>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-sm">
            <thead>
              <tr className="border-b border-[var(--gr-border)] text-left text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">
                <th className="py-3">Hole</th>
                <th className="py-3">Par</th>
                <th className="py-3">SI</th>
            ...
            <tbody>
              {holes.map((hole) => {
                const holeNumber = getHoleNumber(hole);
                const draft = holeDrafts[String(holeNumber)] || { par: "", strokeIndex: "", value: "0" };
                return (
                  <tr key={hole.id} className="border-b border-[var(--gr-border)]/60">
                    <td className="py-3 font-black">{holeNumber}</td>
                    <td className="py-3"><input type="number" className="w-24" value={draft.par} onChange={(event) => setHoleDrafts((drafts) => ({ ...drafts, [String(holeNumber)]: { ...draft, par: event.target.value } }))} /></td>
                    <td className="py-3"><input type="number" className="w-24" value={draft.strokeIndex} onChange={(event) => setHoleDrafts((drafts) => ({ ...drafts, [String(holeNumber)]: { ...draft, strokeIndex: event.target.value } }))} /></td>
                    <td className="py-3"><input type="number" className="w-28" value={draft.value} onChange={(event) => setHoleDrafts((drafts) => ({ ...drafts, [String(holeNumber)]: { ...draft, value: event.target.value } }))} /></td>
                    <td className="py-3 text-right"><button className="btn btn-secondary" disabled={saving} onClick={() => saveHole(hole)}>Save</button></td>
                  </tr>
                );
              })}
            </tbody>
