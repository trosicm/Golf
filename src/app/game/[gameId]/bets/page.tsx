...
export default function BetsPage() {
  ...
  const loadData = async () => {
    ...
    const [gameRes, gameTeamsRes, teamsRes, playersRes, invitesRes] = await Promise.all([
    ...
    const failed = [["games", gameRes], ["game_teams", gameTeamsRes], ["teams", teamsRes], ["players", playersRes], ["game_invites", invitesRes]].find(([, res]: any) => res.error);
  ...
  const teamRows = useMemo(() => {
    return gameTeams.map((gameTeam, index) => {
      ...
      return { id: teamId, name: team?.name || `Team ${index + 1}`, players: playerNames.length ? playerNames.join(" & ") : "Players pending", betsNet, bettingBalance: betsNet };
    });
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto">
      ...
      <section className="card mb-4"><div className="mb-3 flex items-center justify-between gap-3"><div><div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-gold)]">Betting wallets</div><div className="text-sm text-[var(--gr-text-muted)]">Bets affect team balance, not the hole pot.</div></div><div className="rounded-full bg-black px-4 py-2 text-sm font-black text-[var(--gr-sand)]">Open bets {openBets.length}</div></div><div className="grid grid-cols-1 gap-3 md:grid-cols-2">{teamRows.map((team) => <div key={team.id} className="rounded-2xl border border-[var(--gr-border)] bg-[rgba(20,68,55,0.35)] p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-lg font-black text-[var(--gr-sand)]">{team.name}</div><div className="text-xs text-[var(--gr-text-muted)]">{team.players}</div></div><div className={`text-right text-2xl font-black ${team.bettingBalance >= 0 ? "text-success" : "text-danger"}`}>{money(team.bettingBalance)}</div></div><div className="mt-3 text-center text-xs"><div className="rounded-xl border border-[var(--gr-border)] p-3"><div className="text-[var(--gr-text-muted)]">Bets net</div><div className={`font-black ${team.betsNet >= 0 ? "text-success" : "text-danger"}`}>{money(team.betsNet)}</div></div></div></div>)}</div></section>
