...
const carryValue = (hole: any) => n(hole?.carry_in ?? hole?.carryIn);
...
function safeHole(value: any) { return Math.min(18, Math.max(1, n(value, 1))); }
function currentHoleStorageKey(gameId: any) { return `golf-rivals:${gameId}:current-hole`; }
function quickScoreOptions(par: number) { return [
...
async function insertWithFallback(table: string, rows: any[], fallbacks: ((row: any) => any)[]) { const variants = [rows, ...fallbacks.map((fn) => rows.map(fn))]; let lastError: any = null; for (const variant of variants) { const { error } = await supabase.from(table).insert(variant); if (!error) return; lastError = error; } throw lastError; }
...
export default function GameLive() {
  ...
  const resultHoleNumber = (row: any) => { const direct = row?.hole_number ?? row?.holeNumber ?? row?.number; if (direct) return n(direct); const holeId = row?.hole_id ?? row?.holeId; return holeNo(holes.find((hole) => hole.id === holeId)); };
  ...
  const loadData = async (silent = false) => {
    ...
    if (inviteError) { setError(`Could not check invitation: ${inviteError.message}`); setLoading(false); setRefreshing(false); return; }
    const inviteRow = invites?.[0];
    setInvite(inviteRow);
    if (!inviteRow) { setError(`You are not invited to this match. Email: ${email}`); setLoading(false); setRefreshing(false); return; }
    ...
    if (failed) { setError(`${failed[0]}: ${(failed[1] as any).error.message}`); setLoading(false); setRefreshing(false); return; }
  ...
  useEffect(() => { if (!gameId || typeof window === "undefined") return; setStoredHole(safeHole(window.localStorage.getItem(currentHoleStorageKey(gameId)))); }, [gameId]);
  ...
  const saveDraftScore = async (team: any, score: number | string) => {
    ...
    try {
      ...
      setCalculation(null); setMessage(`Saved marker score for ${team.name}: ${grossScore}`);
  ...
  const buyPurchase = async (team: any, type: "mulligan" | "reverse") => {
    ...
    try {
      ...
      setPurchases((current) => [...current, rowWithHoleId]); setCalculation(null); setMessage(type === "mulligan" ? `Mulligan added: ${team.name} +€${amount}` : `Reverse added: ${team.name} +€${amount}`);
    } catch (err: any) { setMessage(`Could not add purchase: ${err?.message || "Unknown Supabase error"}`); } finally { setSaving(false); }
  };
  ...
  const confirmHole = async (mode: "winner" | "carry") => {
    ...
    try {
      ...
      if (!isFinalHole) {
        const nextCarry = mode === "carry" ? liveData.currentPot : 0;
        await supabase.from("holes").update({ carry_in: nextCarry }).eq("game_id", gameId).eq("hole_number", nextHole);
      }
      ...
      setStoredHole(nextHole); setGrossScores({}); setCalculation(null); setMessage(mode === "winner" ? "Winner confirmed. Carry cleared for next hole." : "Carry confirmed. Full pot moves to next hole."); await loadData(true);
  ...
  return (
    <div className="p-3 max-w-3xl mx-auto">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-end gap-2"><div className="rounded-full border border-[var(--gr-border)] px-3 py-2 text-xs font-bold text-[var(--gr-text-muted)]">{invite?.role || "player"}</div><button type="button" className="rounded-full border border-[var(--gr-border)] px-3 py-2 text-xs font-black text-[var(--gr-sand)]" disabled={refreshing || saving} onClick={() => loadData(true)}>{refreshing ? "Refreshing..." : "Refresh Match"}</button><div className="text-[10px] text-[var(--gr-text-muted)]">Auto 30s{lastSync ? ` · ${lastSync.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : ""}</div></div>
      </div>

      <section className="card mb-4 p-0 overflow-hidden"><div className="grid grid-cols-3 border-b border-[var(--gr-border)]"><div className="p-4"><div className="text-xs font-bold uppercase text-[var(--gr-text-muted)]">Hole</div><div className="text-3xl font-black">{liveData.currentHoleNumber}<span className="text-base text-[var(--gr-text-muted)]"> / 18</span></div></div><div className="p-4 text-center border-x border-[var(--gr-border)]"><div className="text-xs font-bold uppercase text-[var(--gr-text-muted)]">Current Pot</div><div className="text-3xl font-black text-[var(--gr-gold)]">€{liveData.currentPot.toFixed(0)}</div></div><div className="p-4 text-right"><div className="text-xs font-bold uppercase text-[var(--gr-text-muted)]">Par / SI</div><div className="text-3xl font-black">{liveData.currentHole.par ?? "-"} / {strokeIndex(liveData.currentHole)}</div></div></div><div className="grid grid-cols-4 gap-2 p-4 text-center text-xs sm:text-sm"><div><div className="text-[var(--gr-text-muted)]">Hole Value</div><div className="font-black text-[var(--gr-gold)]">€{liveData.baseValue.toFixed(0)}</div></div><div><div className="text-[var(--gr-text-muted)]">Carried Over</div><div className="font-black text-[var(--gr-warning)]">€{liveData.carryIn.toFixed(0)}</div></div><div><div className="text-[var(--gr-text-muted)]">Added</div><div className="font-black text-[var(--gr-turf)]">€{liveData.purchaseTotal.toFixed(0)}</div></div><div><div className="text-[var(--gr-text-muted)]">Reverse Price</div><div className="font-black text-[var(--gr-gold)]">€{liveData.currentPot.toFixed(0)}</div></div></div></section>

      <section className="grid grid-cols-1 gap-3 mb-4">{scoringRows.map((team, index) => (<div key={team.id} className={`card mb-0 ${team.canEditScore ? "" : "opacity-75"}`}><div className="flex items-start justify-between gap-3 mb-3"><div><span className={`team-badge team-${(index % 4) + 1}`}>{team.name}</span><div className="mt-2 text-sm text-[var(--gr-text-muted)]">{team.players}</div><div className="mt-1 text-xs text-[var(--gr-gold)]">Marker: {team.markerName}{team.canEditScore ? " · Editable by you" : " · Read only"}</div></div><div className="text-right"><div className="text-xs uppercase text-[var(--gr-text-muted)]">Balance</div><div className={team.balance < 0 ? "text-2xl font-black text-danger" : "text-2xl font-black text-success"}>{money(team.balance)}</div></div></div><div className="grid grid-cols-4 gap-2 mb-4 text-center text-xs"><div className="rounded-xl border border-[var(--gr-border)] p-3"><div className="block text-[var(--gr-text-muted)]">Gross</div><button type="button" disabled={!team.canEditScore || saving} className="mt-1 w-full rounded-xl border border-[var(--gr-border)] bg-[rgba(20,68,55,0.7)] px-2 py-3 text-center text-xl font-black text-[var(--gr-sand)] outline-none disabled:opacity-50" onClick={() => { setPickerTeam(team); setCustomScore(team.grossInput || ""); }}>{team.grossInput || (team.canEditScore ? "Select" : "Locked")}</button><div className="mt-1 text-[var(--gr-gold)]">Net {team.calculatedNet ?? "-"}</div></div><div className="rounded-xl border border-[var(--gr-border)] p-3"><div className="text-[var(--gr-text-muted)]">Bank</div><div className="text-xl font-black">€{team.baseFunds.toFixed(0)}</div></div><div className="rounded-xl border border-[var(--gr-border)] p-3"><div className="text-[var(--gr-text-muted)]">Mulligans</div><div className="text-xl font-black">{team.mulligansLeft}</div></div><div className="rounded-xl border border-[var(--gr-border)] p-3"><div className="text-[var(--gr-text-muted)]">Reverses</div><div className="text-xl font-black">{team.reversesLeft}</div></div></div><div className="flex items-center justify-between text-xs text-[var(--gr-text-muted)] mb-3"><span>Won <b className="text-success">€{team.won.toFixed(0)}</b></span><span>Spent <b className="text-danger">€{team.spent.toFixed(0)}</b></span><span>HCP <b className="text-[var(--gr-sand)]">{team.handicap}</b></span></div><div className="grid grid-cols-2 gap-2"><button className="btn btn-gold" type="button" disabled={saving || team.mulligansLeft <= 0 || team.currentHoleMulligans >= n(game?.max_mulligans_per_team_per_hole, 1)} onClick={() => buyPurchase(team, "mulligan")}>Buy Mulligan €{n(game?.mulligan_price, 50)}</button><button className="btn btn-secondary" type="button" disabled={saving || team.reversesLeft <= 0 || liveData.currentPurchases.some((purchase) => purchaseType(purchase).includes("reverse"))} onClick={() => buyPurchase(team, "reverse")}>Use Reverse €{liveData.currentPot.toFixed(0)}</button></div></div>))}</section>

      <section className="card"><div className="mb-3 flex items-center justify-between gap-3"><div><div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">Actions</div><div className="font-black">Calculate this hole</div></div><div className="text-right text-xs text-[var(--gr-text-muted)]">Economic impact: <span className="font-black text-[var(--gr-gold)]">€{liveData.currentPot.toFixed(0)}</span></div></div>{message && <div className={`mb-3 rounded-2xl border border-[var(--gr-border)] p-3 text-sm font-bold ${message.startsWith("Could not") || message.includes("limit") ? "text-danger" : "text-success"}`}>{message}</div>}{calculation && <div className="mb-3 rounded-2xl border border-[var(--gr-border)] p-3 text-sm">{calculation.type === "error" && <div className="font-bold text-danger">{calculation.message}</div>}{calculation.type === "winner" && <div><div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">Provisional winner</div><div className="text-xl font-black text-success">{calculation.winner.name}</div><div className="text-[var(--gr-text-muted)]">Gross {calculation.winner.grossScore} · Strokes {calculation.winner.strokesReceived} · Net {calculation.winner.netScore}</div></div>}{calculation.type === "tie" && <div><div className="text-xs uppercase tracking-[0.16em] text-[var(--gr-text-muted)]">Tie / Carry</div><div className="text-xl font-black text-[var(--gr-warning)]">{calculation.tiedTeams.map((team) => team.name).join(" vs ")}</div><div className="text-[var(--gr-text-muted)]">Full pot €{liveData.currentPot.toFixed(0)} moves to the next hole.</div></div>}</div>}<div className="grid grid-cols-2 gap-2"><button className="btn btn-gold col-span-2" type="button" onClick={calculateWinner} disabled={saving || !isAdmin}>Calculate Winner</button><button className="btn btn-secondary" type="button" disabled={saving || !isAdmin || !calculation || calculation.type !== "winner"} onClick={() => confirmHole("winner")}>Confirm Winner</button><button className="btn btn-secondary" type="button" disabled={saving || !isAdmin || !calculation || calculation.type !== "tie"} onClick={() => confirmHole("carry")}>Push / Carry</button></div>{!isAdmin && <div className="mt-3 text-xs text-[var(--gr-text-muted)]">Only admin can calculate and confirm the hole.</div>}</section>

      <div className="grid grid-cols-2 gap-2 mb-6"><Link href={`/game/${gameId}/scorecard`} className="btn btn-secondary">Scorecard</Link><Link href={`/game/${gameId}/leaderboard`} className="btn btn-secondary">Leaderboard</Link><Link href={`/game/${gameId}/admin/purchases`} className="btn btn-secondary">Admin Purchases</Link><Link href={`/game/${gameId}/summary`} className="btn btn-gold">Summary</Link></div>
